    ...
    input(e) {
        ...
        if (e.key === '5') {
            if (this.player.usePotion()) {
                this.message = `포션을 사용했습니다! (남은 포션: ${this.player.inventory.potions})`;
            } else {
                if (this.player.inventory.potions <= 0) this.message = "포션이 없습니다!";
                else if (this.player.hp >= this.player.maxHp) this.message = "체력이 이미 가득 찼습니다!";
            }
        }
